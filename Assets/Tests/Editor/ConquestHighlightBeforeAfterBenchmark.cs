...
public class ConquestHighlightBeforeAfterBenchmark
{
    ...
    // 원본(ConqueredChunkBorderRenderer.TraceLoops)은 폐곡선 목록을 호출마다 새로 만들고
    // 반환한다. 버퍼로 재사용하면 실제보다 할당이 적게 나오므로 원본과 같게 맞춘다.
    ...
    private void Release_RefreshHoveredBorder(Vector2Int? chunkCoord)
    {
        ...
        // BuildBorderLoops() — 원본과 동일하게 새 목록을 만들어 반환한다
        CollectBorderEdges(_claimCells);
        List<List<Vector2Int>> loops = TraceLoops();

        for (int i = 0; i < loops.Count && i < _borderLines.Length; i++)
        {
    ...
    //   ② 출시본 · 가드   : 호버 청크가 그대로인 프레임 (커서 정지 또는 같은 청크 안 이동)
    //   ③ 출시본 · 전환   : 호버 청크가 바뀐 프레임 (경계 폐곡선을 다시 만든다)
    //
    //   ②③은 이전 버전에서 "0 B"를 문자열에 그대로 적어 두었으나 측정한 값이 아니었다.
    //   여기서는 두 경로를 각각 잰다.
    // ────────────────────────────────────────────────────────────────
    [Test]
    public void C_GarbagePerFrame()
    {
        ...
        // ── ① 개선 전 — 만든 List를 전부 살려 둔 채 전후 차이를 KEEP으로 나눈다
        ...
        // ── ② 출시본 · 가드에서 반환되는 프레임
        Vector3Int cell = chunkCells[0];
        _selectedChunkCoord = null;
        _lastHoveredChunkCoords.Clear();
        Release_HandleHover(cell); // 첫 진입만 통과시켜 두고, 이후는 전부 가드에서 반환된다
        double guardBytes = MeasureAllocPerCall(
            () => Release_HandleHover(cell), out int guardIterations, out bool guardClean);

        // ── ③ 출시본 · 호버 청크가 바뀌는 프레임 (두 청크를 번갈아)
        Vector2Int chunkA = _chunkCoords[_chunkCoords.Count / 2];
        Vector2Int chunkB = _chunkCoords[_chunkCoords.Count / 2 + 1];
        Vector3Int cellA = _cellsByChunk[chunkA][0];
        Vector3Int cellB = _cellsByChunk[chunkB][0];
        ...
        _selectedChunkCoord = null;
        _lastHoveredChunkCoords.Clear();
        double switchBytes = MeasureAllocPerCall(
            () => Release_HandleHover((flip++ % 2 == 0) ? cellA : cellB),
            out int switchIterations, out bool switchClean);

        Debug.Log($"[AB] ③ 프레임당 가비지 — 개선 전 {legacyBytes:F0} B/프레임 " +
                  $"(60fps 기준 {legacyBytes * 60 / 1024:F1} KB/초)");
        Debug.Log($"[AB]    출시본 · 가드 프레임 {guardBytes:F0} B " +
                  $"(반복 {guardIterations}회{(guardClean ? "" : " · 구간 중 GC 발생 — 하한값")})");
        Debug.Log($"[AB]    출시본 · 전환 프레임 {switchBytes:F0} B " +
                  $"(반복 {switchIterations}회{(switchClean ? "" : " · 구간 중 GC 발생 — 하한값")})");
        Debug.Log("[AB]    ③의 경계 추적은 원본 ConqueredChunkBorderRenderer.TraceLoops()와 동일하게 " +
                  "폐곡선 목록(바깥 List)과 폐곡선마다의 List를 호출마다 새로 만든다. " +
                  "LineRenderer.SetPosition 등 엔진 호출의 네이티브 할당은 이 수치에 포함되지 않는다.");
    }

    // 워밍업으로 내부 버퍼 용량을 키운 뒤, GC가 한 번도 돌지 않은 구간에서만
    // GetTotalMemory(false) 차이를 읽는다. 구간 중 수집이 일어나면 값이 실제보다 작아지므로
    // 반복 횟수를 줄여 다시 잰다. (이 환경의 Mono는 GetAllocatedBytesForCurrentThread()가 0을 돌려준다)
    private double MeasureAllocPerCall(System.Action call, out int usedIterations, out bool clean)
    {
        int[] candidates = { 2000, 500, 100, 20 };
        double lastBytes = 0;
        int lastN = candidates[0];
        ...
        foreach (int n in candidates)
        {
            ...
            lastBytes = (after - before) / (double)n;
            lastN = n;

            if (System.GC.CollectionCount(0) == gen0)
            {
                usedIterations = n;
                clean = true;
                return lastBytes < 0 ? 0 : lastBytes;
            }
        }

        usedIterations = lastN;
        clean = false;
        return lastBytes < 0 ? 0 : lastBytes;
    }
    ...
    public void D_SixtySecondSession()
    {
        ...
        foreach (int cellsPerSecond in new[] { 0, 5, 15, 40 })
        {
            ...
            Debug.Log($"[AB]    커서 {speedLabel,-10} — 재계산 횟수 {legacyCalls,5} → {releaseCalls,4}회 " +
                      ...
                      $"누적 가비지 {legacyGarbageKb,7:F0} KB → 출시본은 C_GarbagePerFrame의 전환 프레임 값 × {releaseCalls}회로 환산할 것");
        }
    }
